...
const VendorSignup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const res = await registerUser({ name: form.name, email: form.email, password: form.password, role: 'vendor' });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          <Link to="/vendor/login" style={{ color: '#000' }}>Already registered? Login here</Link>
        </div>
      </div>
    </div>
  );
